...
export class MathJaxPreamblePluginSettingTab extends PluginSettingTab {
    ...
    _display(serialized: SerializedPreambles): void {
        ...
        new Setting(containerEl)
            ...
            .setDesc(
                'You need to register preamble files before using them. Once registed, you can use them by specifying them (1) as folder preambles (see below) or (2) via the "preamble" note property.',
            )
            .setHeading()
            .addButton(button => {
                button
            ...
            new Setting(containerEl)
                ...
                .addText(text => {
                    text.setPlaceholder('Path to preamble')
                        ...
                        .then(text =>
                            new FileSuggest(this.plugin, text.inputEl).onSelect(
                                ({ path }) => (preamble.path = path),
                            ),
                        )
                ...
                .addExtraButton(button => {
                    button.setIcon('trash').onClick(() => {
                        const index = serialized.preambles.findIndex(
                            ({ path }) => path === preamble.path,
                        );
        ...
        new Setting(containerEl)
            ...
            .addButton(button => {
                button
                    ...
                    .setCta()
                    .onClick(() => {
                        serialized.folderPreambles.push({
                            folderPath: '',
                            preamblePath: '',
                        });
                        this._display(serialized);
        ...
        for (const folderPreamble of serialized.folderPreambles) {
            new Setting(containerEl)
                    ...
                    text.setPlaceholder('Folder path')
                        ...
                        .then(text =>
                            new FolderSuggest(
                                ...
                                ({ path }) =>
                                    (folderPreamble.folderPath = path),
                            ),
                        )
                        .onChange(path => (folderPreamble.folderPath = path));
                })
                .addText(text => {
                    const preamble = serialized.preambles.find(
                        ({ path }) => path === folderPreamble.preamblePath,
                    );
                    text.setPlaceholder('Preamble path')
                        ...
                        .then(text =>
                            new PreambleSuggest(
                                ...
                                folderPreamble.preamblePath =
                                    preamble?.path ?? '';
                            }),
                        )
                        .onChange(preamblePath => {
                            const preamble = serialized.preambles.find(
                                p => p.path === preamblePath,
                            );
                ...
                .addExtraButton(button => {
                    button.setIcon('trash').onClick(() => {
                        const index = serialized.folderPreambles.findIndex(
                            ({ folderPath }) =>
                                folderPath === folderPreamble.folderPath,
                        );
