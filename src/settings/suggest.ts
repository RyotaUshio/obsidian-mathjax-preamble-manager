...
abstract class AbstractPathSuggest<
...
> extends AbstractInputSuggest<T> {
    ...
    selectSuggestion({ path }: T) {
        // @ts-expect-error: super.selectSuggestion calls the callbacks
        // registered via onSelect (alternatively, we can do
        // this.selectCb?.({ path }))
